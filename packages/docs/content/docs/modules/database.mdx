...
| `options.threshold` | `number` | Minimum similarity score (optional) |
| `options.limit` | `number` | Max results (optional) |
| `options.metric` | `DistanceMetric` | `'cosine' \| 'l2' \| 'innerProduct'` (optional) |
| `options.select` | `string[]` | Columns to return (optional) |
...
- `VectorSearchOptions` — table, column, queryVector, threshold, limit, metric, select
