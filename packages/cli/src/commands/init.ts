...
export const initCommand = new Command("init")
	...
	.option("--backend <type>", "Backend framework (fastapi)", "fastapi")
	.option("--frontend <type>", "Frontend framework (nextjs)", "nextjs")
	.action((name: string, options: { backend: string; frontend: string }) => {
		...
		console.log(
			chalk.cyan("  cp .env.example .env          # add your API keys"),
		);
		console.log(
			chalk.cyan("  docker compose up -d           # start Postgres + Redis"),
		);
		console.log(
			chalk.cyan("  cd backend && uv sync          # install Python deps"),
		);
		console.log(
			chalk.cyan("  uv run uvicorn app.main:app --reload  # start backend"),
		);
		console.log(
			chalk.cyan("  cd ../frontend && yarn install  # install frontend deps"),
		);
		console.log(
			chalk.cyan("  yarn dev                        # start frontend"),
		);
		console.log();
	});
